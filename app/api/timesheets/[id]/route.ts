...
export async function PUT(
  ...
  try {
    ...
    for (const row of normalized) {
      if (!row.employeeName)
        errors.push("Employee name is required for all rows.");
      ...
      if (needsTime) {
        if (!row.timeIn)
          errors.push(
            `Time In is required for ${row.employeeName} on ${row.date}.`,
          );
        if (!row.timeOut)
          errors.push(
            `Time Out is required for ${row.employeeName} on ${row.date}.`,
          );
        if (
          row.totalHours === null ||
          row.totalHours === undefined ||
          Number.isNaN(Number(row.totalHours))
        ) {
          errors.push(
            `Hours are required for ${row.employeeName} on ${row.date}.`,
          );
        }
      }
      ...
      if (daySet.has(row.date)) {
        errors.push(
          `Duplicate date ${row.date} for ${row.employeeName}. Each employee needs unique dates.`,
        );
      }
      daySet.add(row.date);
      if (daySet.size > 20) {
        errors.push(
          `Timesheets support up to 20 days per employee. ${row.employeeName} exceeds this limit.`,
        );
      }
    ...
    if (errors.length) {
      return NextResponse.json(
        { ok: false, error: Array.from(new Set(errors)).join(" ") },
        { status: 400 },
      );
    }
    ...
    if (!finalRows.length) {
      return NextResponse.json(
        { ok: false, error: "No valid rows to save." },
        { status: 400 },
      );
    }

    const employeeNamesForValidation = [
      ...new Set(finalRows.map((r) => r.employeeName)),
    ];
    ...
    finalRows.forEach((row) => {
      ...
      if (
        existingDept &&
        row.dept &&
        row.dept.trim().toLowerCase() !== existingDept.trim().toLowerCase()
      ) {
        errors.push(
          `Department mismatch for ${row.employeeName}. Existing: ${existingDept}`,
        );
      }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const employeeMap = new Map(
        employees.map((emp) => [emp.employeeName, emp]),
      );
      ...
      await tx.timesheetRow.createMany({
        data: finalRows.map((row) => {
          ...
          const totalHours =
            status === "absent" ? null : (row.totalHours ?? null);
