    ...
    <section className="rounded-3xl border border-[var(--border)] bg-[var(--panel)]/90 p-6 shadow-[0_24px_70px_rgba(16,40,94,0.1)]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">Full day, half day, absent counts and worked hours per employee.</p>
        </div>
          ...
          <table className="min-w-[800px] w-full text-sm">
            <thead className="bg-[var(--surface)] text-[var(--muted)]">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.24em]">Full Day</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.24em]">Half Day</th>
            ...
            <tbody className="divide-y divide-[var(--border)]/70 text-[var(--foreground)]">
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-4 py-4 text-center text-[var(--muted)]">
                ...
                rows.map((row) => (
                  <tr key={`${row.employeeName}-${row.userId ?? ""}`} className="hover:bg-[var(--surface)]/60">
                    ...
                    <td className="px-4 py-3 text-[var(--muted)]">{row.attendanceDays}</td>
                    <td className="px-4 py-3 text-[var(--muted)]">{row.halfDays}</td>
                    <td className="px-4 py-3 text-[var(--muted)]">{row.absentDays}</td>
                    <td className="px-4 py-3 text-[var(--muted)]">{row.leaveDays}</td>
